      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-3 group">
            ...
            <div>
              <span className="block font-bold text-lg text-navy-950 leading-tight">1st Blessed</span>
              <span className="block text-xs font-medium text-gold-600 tracking-wider uppercase">Adult Family Home</span>
            </div>
          </Link>
            ...
            {NAV_LINKS.map(({ name, path }) => (
              <Link
                ...
                className={`text-sm font-medium transition-colors hover:text-gold-600 ${
                  pathname === path ? 'text-gold-600 font-semibold' : 'text-navy-950/80'
                }`}
              >
                {name}
              </Link>
          ...
          <div className="hidden lg:flex items-center space-x-4">
            ...
            <Button to="/contact" variant="gold" size="sm">Schedule a Tour</Button>
          </div>

          <div className="flex items-center space-x-2 md:hidden">
            <Button to="/contact" variant="gold" size="sm" className="text-xs px-3 py-1.5">Tour</Button>
            <button
        ...
        <div className="md:hidden fixed inset-x-0 top-[65px] bg-white border-b border-gray-200 shadow-xl p-6 animate-in slide-in-from-top-4">
          <nav className="flex flex-col space-y-4">
            {NAV_LINKS.map(({ name, path }) => (
              <Link
                ...
                className={`text-base font-medium py-2 px-3 rounded-lg ${
                  pathname === path ? 'bg-gold-50 text-gold-600 font-semibold' : 'text-navy-950 hover:bg-gray-50'
                }`}
              >
                {name}
              </Link>
            ...
            <div className="pt-4 border-t border-gray-100 flex flex-col space-y-3">
              ...
              <Button to="/contact" variant="gold" size="md" className="w-full">Schedule a Tour</Button>
            </div>
          </nav>
        </div>
      )}
